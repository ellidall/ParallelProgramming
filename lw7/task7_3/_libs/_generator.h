...
struct Promise
{
    ...
    // выяснить и должить про suspend_newer
    template<typename U>
    std::suspend_always yield_value(U&& value)
    {
        if (m_value_ptr)
        {
            *m_value_ptr = std::forward<U>(value);
        }
        return {};
    }
...
{
public:
    // дорабтать генератор так, чтобы генерировать объекты которые не имеют конструктора по умолчанию
    // как сделать генератор так, чтобы генератор не хранил значение
    ...
    struct iterator
    {
        ...
        {
            currentValue.reset();
            h.promise().m_value_ptr = &currentValue.emplace();
